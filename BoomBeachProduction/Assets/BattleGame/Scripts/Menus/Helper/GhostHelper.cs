...
public class GhostHelper : MonoBehaviour {					//the helper assisting the user at first run
	...
	public Text LbExplain;								//displayed in the middle of the screen
	...
	*/
	...
	{
        if(!Stats.Instance.ShowTutorial || Stats.Instance.TutorialCitySeen)
        {
            return;
        }
	...
	public void OnUnits()
	{
        ...
        ShopController.Intance.OpenUnitsCategory();
	...
	public void OnShop()
	{
        if(!Stats.Instance.ShowTutorial || Stats.Instance.TutorialCitySeen || EventSystem.current.currentSelectedGameObject)
        {
            return;
        }
