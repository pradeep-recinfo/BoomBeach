...
public class SaveLoadBattle : SaveLoadBase {
	...
	public string 											//http://recinfotech.com/costin/admin.php
		serverAddress = "https://recinfotech.com/UA/pvp-server-sync-v6/",		
	...
	{
		isBattleMap = true;
		// campaignNo = new string[GameUnitsSettings_Handler.s.campaignNo];
				...
				else
				{
					...
					if(w2.text != null && w2.text != "")
					{
						if(w2.text.Contains("###StartofFile###") && w2.text.Contains("###EndofFile###"))
						{
							...
							MessageController.Instance.DisplayMessage("Uploaded attack results to "  + battleMapID + attackExt );
						}
						else
						{
		...
		else
		{
			...
			if(w2.text != null && w2.text != "")
			{
				...
				{			
					...
					MessageController.Instance.DisplayMessage("License code not added in Map01\nor downloaded file corrupted."); 
